...
const SHEET_URL =
  "https://script.google.com/macros/s/AKfycbwhnXVNAzzz2dqPu-tL9MLD66fCVylfeJ8OF4dYZbu4UzZ48g5iA-pKRnTAMSoMgMmk/exec";
...
const Testimonials = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        formatted.sort((a: Review, b: Review) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSheetReviews((prev) => [newReview, ...prev]);
      // Make sure the new review is visible
  ...
  const handleShowLess = () => {
    ...
    // Scroll back up to the top of the reviews grid
    reviewGridRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  const visibleReviews = sheetReviews.slice(0, visibleCount);
  const hasMore = visibleCount < sheetReviews.length;
    ...
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          {/* Review count badge */}
          {!isLoading && sheetReviews.length > 0 && (
            <p className="text-sm text-gray-400 mb-4">
              {sheetReviews.length} verified review{sheetReviews.length !== 1 ? "s" : ""}
            </p>
          )}
        ...
        {isLoading ? (
          ...
          <>
            {/* Testimonials Grid */}
            <div ref={reviewGridRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {visibleReviews.map((testimonial, index) => (
                <div
                  ...
                  style={{
                    ...
                    animationDelay: `${(index % REVIEWS_PER_PAGE) * 60}ms`,
                  }}
                >
                  {/* Rating Stars */}
                  <div className="flex mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-gray-400">{formatDate(testimonial.Date)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {sheetReviews.length > REVIEWS_PER_PAGE && (
              <div className="text-center mt-10">
                {/* Progress indicator */}
                <p className="text-sm text-gray-400 mb-4">
                  Showing {Math.min(visibleCount, sheetReviews.length)} of {sheetReviews.length} reviews
                </p>
                ...
                <div className="w-48 h-1.5 bg-gray-200 rounded-full mx-auto mb-6 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min((visibleCount / sheetReviews.length) * 100, 100)}%`,
                    }}
                  />
                </div>
                ...
                <div className="flex items-center justify-center gap-3">
                  {hasMore && (
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-white border-2 border-primary text-primary hover:bg-primary hover:text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 shadow-sm hover:shadow-md"
                    >
                      <span>Show More Reviews</span>
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                      </svg>
                      <span className="bg-primary text-white group-hover:bg-white group-hover:text-primary text-xs px-2 py-0.5 rounded-full ml-1 font-bold border border-primary">
                        +{Math.min(REVIEWS_PER_PAGE, sheetReviews.length - visibleCount)}
                      </span>
                    </button>
                  )}
                  ...
                  {isExpanded && (
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-600 px-6 py-3 rounded-lg font-semibold transition-all duration-200"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
