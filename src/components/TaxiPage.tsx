...
const routeData: Record<string, RouteInfo> = {
  'jaipur-delhi': {
    ...
    distance: '280 km',
    duration: '5-6 hours',
    description: 'Travel comfortably from the Pink City to the Capital of India. The route takes you through the scenic NH48 highway with excellent road conditions.',
    attractions: ['Neemrana Fort Palace', 'Vrindavan', 'Kingdom of Dreams Gurgaon']
  },
  'jaipur-agra': {
    from: 'Jaipur',
    to: 'Agra',
    distance: '238 km',
    duration: '4-5 hours',
    description: 'Experience the journey from the Pink City to the city of Taj Mahal. The route passes through beautiful Rajasthani landscapes and historic sites.',
    attractions: ['Abhaneri Stepwell (Chand Baori)', 'Bharatpur Bird Sanctuary', 'Fatehpur Sikri', 'Taj Mahal', 'Agra Fort']
  },
  'jaipur-udaipur': {
    from: 'Jaipur',
    to: 'Udaipur',
    distance: '395 km',
    duration: '6-7 hours',
    description: 'Journey from the Pink City to the City of Lakes. Enjoy the beautiful Aravalli hills and traditional Rajasthani villages along the way.',
    attractions: ['Chittorgarh Fort', 'Pushkar Temple', 'Ajmer Dargah', 'Nathdwara Temple']
  },
  'jaipur-jodhpur': {
    from: 'Jaipur',
    to: 'Jodhpur',
    distance: '335 km',
    duration: '5-6 hours',
    description: 'Travel from the Pink City to the Blue City. The route showcases the transition from urban Rajasthan to the Thar Desert region.',
    attractions: ['Ajmer Sharif Dargah', 'Pushkar Lake', 'Kishangarh Fort', 'Mehrangarh Fort']
  },
  'jaipur-jaisalmer': {
    from: 'Jaipur',
    to: 'Jaisalmer',
    distance: '560 km',
    duration: '8-9 hours',
    description: 'Embark on an epic journey from the Pink City to the Golden City in the heart of the Thar Desert. Experience the changing landscapes of Rajasthan.',
    attractions: ['Jodhpur', 'Osiyan Temples', 'Pokaran Fort', 'Jaisalmer Fort', 'Sam Sand Dunes']
  }
}
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            ...
            <p className="text-gray-600 text-center mb-12 max-w-2xl mx-auto">
              Choose from our most popular outstation taxi routes with transparent pricing and professional drivers
            </p>
                  ...
                  <div className="relative h-40 bg-gradient-to-br from-secondary to-blue-900">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-white text-center">
                        ...
                        <div className="text-xl font-bold">{info.from} → {info.to}</div>
                      </div>
                    </div>
                  </div>
                  <div className="p-6">
                    <div className="flex justify-between items-center mb-3">
                      <span className="text-primary font-semibold">{info.distance}</span>
        ...
        <section className="py-16 bg-white">
          <div className="container mx-auto px-4">
            ...
            <p className="text-gray-600 text-center mb-12 max-w-2xl mx-auto">
              Well-maintained vehicles with experienced drivers for your comfortable journey
            </p>
            
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {taxiFleet.filter(t => t.category !== 'Tempo').map((taxi) => (
                <div key={taxi.id} className="bg-gray-50 rounded-xl p-6 shadow hover:shadow-lg transition-all">
                  <div className="text-4xl mb-4 text-center">🚗</div>
                  <h3 className="text-lg font-bold text-gray-800 text-center mb-2">{taxi.name}</h3>
                  <p className="text-primary font-semibold text-center mb-2">₹{taxi.perKm}/km</p>
                  <p className="text-gray-500 text-center text-sm mb-3">{taxi.capacity} Seater</p>
                  <div className="flex flex-wrap gap-1 justify-center">
                    {taxi.features.slice(0, 3).map((feature, idx) => (
                      <span key={idx} className="text-xs bg-gray-200 px-2 py-1 rounded">{feature}</span>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                { icon: '👨‍✈️', title: 'Professional Drivers', desc: 'Experienced and verified drivers' },
                { icon: '🚗', title: 'Clean Vehicles', desc: 'Well-maintained and sanitized cars' },
                { icon: '📍', title: 'GPS Tracking', desc: 'Real-time location tracking available' }
              ].map((item, idx) => (
        ...
        <section className="py-16 bg-secondary text-white">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-2xl md:text-3xl font-bold mb-4">Ready to Book Your Taxi?</h2>
            <p className="text-gray-300 mb-8">Contact us now for the best rates and instant booking</p>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-800 mb-4">Route Not Found</h1>
          <Link to="/outstation-taxi" className="text-primary hover:underline">View All Routes</Link>
        </div>
      </div>
    )
  }

  const filteredTaxis = selectedCategory === 'all' 
    ? taxiFleet 
    : taxiFleet.filter(taxi => taxi.category.toLowerCase() === selectedCategory.toLowerCase())
      ...
      <div className="relative bg-gradient-to-r from-secondary via-blue-900 to-secondary text-white py-20">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <nav className="text-sm mb-4">
            ...
            <span className="mx-2">/</span>
            <span>{routeInfo.from} to {routeInfo.to}</span>
          </nav>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                <span className="bg-primary text-secondary px-3 py-1 rounded mr-3 text-sm">BEST PRICE</span>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-primary to-yellow-500 text-secondary">
                      <th className="px-4 py-3 text-left rounded-tl-lg">Vehicle</th>
                      ...
                      <th className="px-4 py-3 text-center rounded-tr-lg">Est. Fare</th>
                    </tr>
                  </thead>
                  <tbody>
                    {taxiFleet.map((taxi, index) => (
                      <tr key={taxi.id} className={`border-b ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 transition-colors`}>
                        <td className="px-4 py-4">
                          <div className="font-semibold text-gray-800">{taxi.name}</div>
                          <div className="text-sm text-gray-500">({taxi.category})</div>
                        </td>
                        <td className="px-4 py-4 text-center">
                          <span className="text-green-600 font-bold">₹{taxi.perKm}/-</span>
                        </td>
                        <td className="px-4 py-4 text-center text-gray-700">₹{taxi.nightCharge}/-</td>
                        <td className="px-4 py-4 text-center text-gray-700">{taxi.capacity}</td>
                        <td className="px-4 py-4 text-center">
                          <span className="text-primary font-bold">₹{calculateFare(taxi.perKm).toLocaleString()}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <p className="text-sm text-gray-500 mt-4">* Toll tax, parking, and state permit charges are extra</p>
            </div>
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-6">
              ...
              <span className="ml-3 bg-red-500 text-white text-sm px-3 py-1 rounded-full animate-pulse">Get upto 10% off</span>
            </h2>
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              {filteredTaxis.map((taxi) => (
                <div key={taxi.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow group">
                  ...
                  <div className="p-5">
                    <h3 className="text-xl font-bold text-gray-800 mb-3">{taxi.name}</h3>
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Per Km Charge</p>
                        <p className="text-lg font-bold text-green-600">₹{taxi.perKm}</p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Seating</p>
                        <p className="text-lg font-bold text-gray-800">{taxi.capacity}</p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Night Charge</p>
                        <p className="text-lg font-bold text-gray-800">₹{taxi.nightCharge}</p>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-xs text-gray-500">Est. Fare</p>
                        <p className="text-lg font-bold text-primary">₹{calculateFare(taxi.perKm).toLocaleString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <button 
                        ...
                        className="flex-1 bg-primary text-secondary font-semibold py-3 rounded-lg hover:bg-yellow-500 transition-colors"
                      >
                        Book Now
                      </button>
                      <a 
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <div className="flex items-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-800">{routeInfo.distance}</p>
                  </div>
                </div>
                <div className="flex items-center p-4 bg-green-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-800">{routeInfo.duration}</p>
                  </div>
                </div>
              </div>

              <h3 className="text-xl font-bold text-gray-800 mb-4">Attractions Along the Way</h3>
            ...
            <div className="bg-gradient-to-r from-secondary to-blue-900 text-white rounded-xl shadow-lg p-6">
              <h2 className="text-2xl font-bold mb-6">Why Choose Shree Shyam Tour and Travels?</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={idx} className="flex items-center bg-white/10 p-4 rounded-lg">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 sticky top-24">
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                <p className="text-gray-600 text-center mb-4">Or call us directly</p>
                <a 
                  href="tel:+919785560707"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-primary to-yellow-500 text-secondary p-4 rounded-t-xl flex justify-between items-center">
              ...
              <button onClick={() => setShowEnquiryForm(false)} className="text-2xl">&times;</button>
            </div>
            <div className="p-6">
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                <p className="text-sm text-gray-500">Route</p>
                <p className="font-bold text-gray-800">{routeInfo.from} → {routeInfo.to}</p>
                <p className="text-sm text-gray-500 mt-2">Estimated Fare</p>
                <p className="font-bold text-2xl text-primary">₹{calculateFare(selectedTaxi.perKm).toLocaleString()}</p>
              </div>
              <form className="space-y-4">
                <input type="text" placeholder="Your Name *" className="w-full px-4 py-3 border rounded-lg" required />
                <input type="tel" placeholder="Mobile Number *" className="w-full px-4 py-3 border rounded-lg" required />
                <input type="date" className="w-full px-4 py-3 border rounded-lg" required />
                <input type="time" className="w-full px-4 py-3 border rounded-lg" />
                <textarea placeholder="Pickup Address" rows={2} className="w-full px-4 py-3 border rounded-lg"></textarea>
                <button type="submit" className="w-full bg-primary text-secondary font-bold py-3 rounded-lg hover:bg-yellow-500">
                  Confirm Booking
                </button>
              </form>
            </div>
